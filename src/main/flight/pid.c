...
//        float pidsum = pterm + dterm;
        float pidsum = pterm;
        currentPidSetpoint[axis] = pidsum; // attitude pid pidsum becomes the setpoint into to the rate pid controller
    }
    ...
    currentPidSetpoint[FD_YAW] += gravity_vector[FD_YAW] * yaw_setpoint;
